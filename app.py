...
def get_user(db: Session, username: str):
    return data_manager.get_user_by_username(username, db)
...
@app.post("/skills")
async def add_skill(skill: SkillCreate, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
...
async def start_training(training: TrainingCreate, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
...
async def evaluate_skill(skill_id: int, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db), level: int = Form(...)):
    # Check if the user has the skill
    skill_statement = select(Skill).where(Skill.id == skill_id, Skill.user_id == current_user.id)
    skill = db.exec(skill_statement).first()
...
async def complete_training(skill_id: int, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    # Find the training
